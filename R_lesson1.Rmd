...
## Two tips
Remember: 
- RStudio (and other IDEs) has a very useful feature called "code completion": use the Tab key (↹) to autocomplete the full name of an object or a function, or a file/folder location
...
Let’s first create 3 vectors (must be of the same length)
```{r eval=TRUE}
...
```
You can add a column to a __data frame__ assigning a vector to it
```{r eval=FALSE}
...
Sam,39,briefly
```
When you load a file check if the table contains a header
Possible mistakes:
...
You can download the table at the link, and save it into a folder of your choice

https://drive.google.com/file/d/1UMmE_KZial63WJVleg4xbX6GXCtCTd45/view?usp=sharing

Otherwise you can download it from the gdrive folder of the course or from github
...
Or navigate to the folder using "files"
---
## Reading a table
